...
describe('sendWebhookMessage', () => {
  ...
  test.each([
    [MessageFormat.Text, { [MessageFormat.Text]: 'Hello' }],
    [MessageFormat.Markdown, { [MessageFormat.Markdown]: 'Hello' }],
  ] as const)(
  ...
  test('should return a failure for a non-200 response', async () => {
    ...
    await expect(
      sendWebhookMessage(WEBHOOK, MessageFormat.Text, 'Hello', 10_000),
